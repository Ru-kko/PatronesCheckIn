...
interface FlightStateChangeOptions {
  delayMinutes?: number;
}
...
export class FlightMediator {
  ...
  getEnabledStates(): FlightStateName[] {
    return this.flightStateController.getEnabledTransitions();
  }
  ...
  changeState(nextState: FlightStateName, options: FlightStateChangeOptions = {}): void {
    const enabled = this.getEnabledStates();

    if (!enabled.includes(nextState)) {
      throw new Error(`Transicion invalida: ${this.getState()} -> ${nextState}`);
    }

    switch (nextState) {
      case "Delayed": {
        const minutes = options.delayMinutes ?? 15;
        this.delay(minutes);
        break;
      }
      case "Cancelled":
      ...
      case "New":
      default:
        throw new Error(`No existe una accion de cambio soportada para ${nextState}`);
    }
  }
}